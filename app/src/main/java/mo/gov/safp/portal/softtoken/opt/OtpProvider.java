...
public class OtpProvider implements OtpSource {
  ...
  @Override
  public List<AccountDb.AccountIndex> enumerateAccounts() {
    return mAccountDb.getAccounts();
  }

  @Override
  public String getNextCode(AccountDb.AccountIndex account) throws OtpSourceException {
  ...
  @Override
  public String respondToChallenge(AccountDb.AccountIndex account, String challenge)
  ...
  private String getCurrentCode(AccountDb.AccountIndex account, byte[] challenge) throws OtpSourceException {
    // Account is required.
    if (account == null) {
      throw new OtpSourceException("No account");
    }

    AccountDb.OtpType type = mAccountDb.getType(account);
    ...
    if (type == AccountDb.OtpType.TOTP) {
      ...
      otpState = mTotpCounter.getValueAtTime(Utilities.millisToSeconds(mTotpClock.nowMillis()));
   ...
   * @param index {@link } identifying the user.
   * @return the secret key as base32 encoded string.
   */
  String getSecret(AccountDb.AccountIndex index) {
